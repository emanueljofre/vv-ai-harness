...
| `LocalizationResources`            | `object`   | **v6.1+, customer-scoped.** Localization resource map. Present as empty `{}` on vv5dev/WADNR (2026-04-10); **absent (undefined) on vv5dev/EmanuelJofre (2026-04-20)** despite the same platform version — presence is driven by a per-customer localization feature, not platform version alone. Not present on v5.1 vvdemo. Keys unknown — no configured localization observed yet. | `{}`                                     |
| `LocalizeString`                   | `function` | **v6.1+, customer-scoped.** String localization helper. Present on vv5dev/WADNR alongside `LocalizationResources`; absent on vv5dev/EmanuelJofre. Tracks the same per-customer feature toggle as `LocalizationResources`.                                                                                                                                                            | —                                        |
...
| ---------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GetFieldValue(fieldName)`               | 1      | Returns the processed field value. For calendar fields, applies `getCalendarFieldValue()` which may transform the raw value (FORM-BUG-5 adds fake Z for Config D). **For non-existent fields, returns `""` silently** (no error thrown). |
...
| `getSaveValue(date, enableTime, ignoreTimezone, useLegacy)` | `function` | Converts a Date object to storage format string. Strips Z suffix. For date-only (`enableTime=false`): extracts the **UTC date** via `toISOString()` → takes date portion → produces `"yyyy-MM-dd"`. This is the mechanism behind FORM-BUG-7: IST local midnight = previous UTC day → stores wrong date (DB dump: DateTest-000084 Field7 = `2026-03-14` for IST user who picked March 15). For DateTime: produces `"yyyy-MM-ddTHH:mm:ss"`. **Empirically verified**: produces identical output for `ignoreTZ=true` and `ignoreTZ=false` given the same Date object — the flag affects display only, not storage (DB dump 2026-04-06). |
| `parseDateString(value, enableTime, ignoreTimezone)`        | `function` | Parses a date string during form load. In V1, strips Z and re-parses as local time (FORM-BUG-1).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
