...
| `VV.Form` properties       | 26 properties                                                                       | **Varies by customer**: 26 on vv5dev/EmanuelJofre (2026-04-20), 28 on vv5dev/WADNR (adds `LocalizationResources`, `LocalizeString`, `CurrentLanguageCode`, `FormLanguage`) — customer-scoped localization feature, not a pure Kendo-version property |
...
| `LocalizationResources`    | `undefined`                                                                         | **Customer-scoped**: `undefined` on vv5dev/EmanuelJofre (2026-04-20); `{}` empty object on vv5dev/WADNR — same server + same Kendo v2, so presence is driven by a per-customer localization feature, not platform version                            |
...
// true  → V2 (active when ?ObjectID= param is present, modelId is set, or the Central Admin "Use Updated Calendar Control Logic" checkbox is on at database scope)
```
...
- Central Admin **Database-scope** "Use Updated Calendar Control Logic" checkbox (pushed via `setUserInfo()`) or non-empty `modelId` → V2 (`true`). Database scope wins over Customer scope. See [`docs/architecture/visualvault-platform.md § Central Admin`](../architecture/visualvault-platform.md#central-admin-cross-customer-control-panel).
...
**V2 init path limitation**: The flag resets to `false` on page reload. Console activation works for post-load operations (e.g., testing `GetFieldValue` under V2 — see TC-8-V2), but cannot test the V2 **init path** (`initCalendarValueV2`) because the flag must be `true` before the form loads. Testing the init path requires either a valid Object View context (`?ObjectID=` with a real object) or the Central Admin checkbox enabled at database scope for the target customer (which is the case on vv5dev/EmanuelJofre — every bare template load there runs V2).

**Full-V2 save pipeline normalizes raw storage to ISO-with-Z for every config** (verified 2026-04-20 on vv5dev/EmanuelJofre, IST-chromium, SFV probe on all 8 A–H configs): under V2 driven by the DB-scope Central Admin toggle (not console activation), `SetFieldValue` stores the raw partition value as `YYYY-MM-DDTHH:mm:ss.000Z` regardless of `enableTime`, `ignoreTimezone`, or `useLegacy`. This collapses three V1 format distinctions at the storage layer:

- **Date-only vs DateTime**: V1 stored Config A/B as `"2026-03-15"`; full-V2 stores `"2026-03-15T00:00:00.000Z"` (or `"2026-03-14T18:30:00.000Z"` in UTC+ when FORM-BUG-7 fires — see below). DateTime configs (C/D/G/H) also gain the `.000Z` suffix that V1 omitted on naive-local values.
- **Legacy vs non-legacy raw**: V1 legacy (`useLegacy=true`, E–H) stored naive values and skipped the save transformation; full-V2 stores the same `.000Z`-suffixed value as non-legacy. **V1 legacy immunity to the GFV fake-Z (FORM-BUG-5) becomes structurally moot under full-V2** — there is no transformation to skip, and raw already carries the Z.
- **GFV transformation**: Under full-V2, `GetFieldValue` returns the raw partition value for every config (api === raw). The V1 `getCalendarFieldValue()` pipeline is bypassed. This is the same behavior as console-activated V2 (GFV returns raw), but the raw itself now carries `.000Z`, so GFV output is `"…T00:00:00.000Z"` rather than the V1 naive `"…T00:00:00"` example shown in the [GFV return by config](#getfieldvalue-return-by-config) table below.

**FORM-BUG-7 under full-V2**: Date-only TZ-aware configs (A, E) in UTC+ timezones still exhibit the day shift, but the stored value is now the UTC-shifted ISO datetime rather than a truncated date string. Example: SFV `"03/15/2026"` in IST on Config A stores `"2026-03-14T18:30:00.000Z"` (IST midnight → UTC previous day, 18:30). `ignoreTimezone=true` (B, D, F, H) still prevents the day shift (stores `"2026-03-15T00:00:00.000Z"`), so it remains the primary FORM-BUG-7 mitigation under V2 as well as V1. Live evidence: [cat11-save-BRT-load-IST-vv5dev-2026-04-20.md](../../projects/emanueljofre-vv5dev/testing/date-handling/forms-calendar/runs/cat11-save-BRT-load-IST-vv5dev-2026-04-20.md).

**Console-activated V2 differs from full-V2 at the save layer**: the flag-flip approach tests V2 GFV (read pipeline) but leaves the save pipeline under V1 (raw was stored before the flag was toggled). Full-V2 requires the DB-scope toggle active **before** form init — that is the case on vv5dev/EmanuelJofre. Any V2 expected-value baseline in tests must specify which of the two modes was captured.
...
| FORM-BUG-8 | V2 `SetFieldValue(name, '' \| null)` hangs indefinitely         | `useUpdatedCalendarValueLogic=true` (V2), Configs A / C / D observed   | Medium — under V2 (Kendo v2 + V2 calendar logic), calling `VV.Form.SetFieldValue(fieldName, '')` or `SetFieldValue(fieldName, null)` never resolves. The browser `page.evaluate` stalls; there is no error, no return, no timeout from VV itself — the only way out is the caller's own timeout. Confirmed 2026-04-20 on vv5dev (build `20260418.1`, prog `6.1.20260416.1`) in Playwright runs — 4 cat-12 edge-case tests all hit this on Configs A, C, D with empty-string or null input. V1 is unaffected (returns promptly, storing either empty/invalid value or raising FORM-BUG-6 on read). **Workaround**: guard with a per-call timeout (e.g., `Promise.race` with 10–15s); do not pass empty/null to SFV under V2. Root cause TBD — likely an async continuation in the V2 pipeline that waits for a non-empty transformed value forever.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
All 16 known bugs exist on both VV environments but **manifest differently** due to VV version/Kendo version differences. First cross-environment regression: 2026-04-09 (252 tests, 4 TZ × chromium, vv5dev build 20260130.1). Second regression: 2026-04-10 (116 BRT-chromium tests, WADNR test harness — **116/116 PASS, identical to EmanuelJofre**). Cross-environment differential investigation (Cat 14–16) in progress — see `research/date-handling/forms-calendar/matrix.md`.
