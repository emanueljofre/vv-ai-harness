...
Workflows matrix: `research/date-handling/workflows/matrix.md` (new 2026-04-20)
Scheduled Processes matrix: `research/date-handling/scheduled-processes/matrix.md` (new 2026-04-20)
...
| `cat-10-ws-input.spec.js`         | Category 10 — web service input (postForms → browser load → capture raw/api/display for CB-8 cross-layer shift)                                                                                                                                                                                                                                             |
...
| `cat-13-db-storage.spec.js`       | Category 13 — DB storage via REST API (apiWriteRead, saveAndApiRead, roundtripSaveAndApiRead, querySameLogicalDate)                                                                                                                                                                                                                                         |
...
| `audit-kendo-version.spec.js`     | Category 15 — cross-env Kendo widget comparison (VV.Form properties, fieldMaster, widget options, parsing/formatting) — **original hardcoded audit, superseded by `cat-15-audit.spec.js` for new captures; kept for v1/v2 regression until consolidation**                                                                                                  |
| `cat-15-audit.spec.js`            | Category 15 — parameterized audit spec. Iterates Cat 15 entries in `test-data.js`, dispatches to named capture functions (`vvCore`, `fieldMasterByConfig`, `kendoGlobal`, `widgetOptsByConfig`, `sfvWidget`, `maskScan`), records observed + `expectedPerEnv` as `audit-data` annotations. Data-collection mode — no per-entry assertion on specific values |
| `dash-filter.spec.js`             | DB-5 — dashboard SQL filter tests (WHERE clause on date fields)                                                                                                                                                                                                                                                                                             |
| `dash-sort.spec.js`               | DB-4 — dashboard column sort order verification (ascending/descending), 4 slot-prefixed tests (`db-4-f7-asc/desc`, `db-4-f6-asc/desc`)                                                                                                                                                                                                                      |
...
| `dash-accuracy.spec.js`           | DB-2 — dashboard date accuracy; SQL-filter per config's canonical WS-1 stored value (`3/15/2026` date-only, `3/15/2026 2:30 PM` datetime) then assert grid renders it verbatim                                                                                                                                                                              |
| `dash-wrong-date.spec.js`         | DB-3 — wrong-date propagation; `beforeAll` POSTs 8 shifted-value records via the Forms API, then filters/asserts each config's grid cell renders the shifted string (`3/14/2026` / `3/14/2026 11:00 PM`)                                                                                                                                                    |
| `dash-tz-independence.spec.js`    | DB-8 — server-render TZ independence; spins up 3 parallel browser contexts (BRT/IST/UTC0) in one test and asserts byte-identical grid values across all three                                                                                                                                                                                               |
...
| Config | enableTime | ignoreTimezone | useLegacy | Field (vvdemo) | Field (vv5dev)           | What It Tests                                            |
| :----: | :--------: | :------------: | :-------: | -------------- | ------------------------ | -------------------------------------------------------- |
|   A    |   false    |     false      |   false   | Field7         | dateTzAwareV2Empty       | Date-only baseline. FORM-BUG-7 surface in UTC+           |
|   B    |   false    |      true      |   false   | Field10        | dateLocalV2Empty         | Date-only + ignoreTimezone. Same FORM-BUG-7 exposure     |
|   C    |    true    |     false      |   false   | Field6         | dateTimeUtcV2Empty       | DateTime with timezone. UTC-aware storage                |
|   D    |    true    |      true      |   false   | Field5         | dateTimeLocalV2Empty     | **Primary bug surface**: FORM-BUG-5 (fake Z), FORM-BUG-6 |
|   E    |   false    |     false      |   true    | Field12        | dateTzAwareLegacyEmpty   | Legacy date-only. FORM-BUG-2, FORM-BUG-4                 |
|   F    |   false    |      true      |   true    | Field11        | dateLocalLegacyEmpty     | Legacy date-only + ignoreTimezone                        |
|   G    |    true    |     false      |   true    | Field14        | dateTimeUtcLegacyEmpty   | Legacy DateTime                                          |
|   H    |    true    |      true      |   true    | Field13        | dateTimeLocalLegacyEmpty | Legacy DateTime + ignoreTimezone                         |

Field names resolve at runtime via `FIELD_MAP_BY_CUSTOMER[vvConfig.customerKey]` in `testing/fixtures/vv-config.js`. The active customer's map is selected by `.env.json` `activeCustomer`.
