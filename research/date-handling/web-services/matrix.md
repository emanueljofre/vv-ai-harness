...
The regression pipeline runs **multi-TZ for actions where server TZ changes outcomes** and **BRT-only for the rest**. Matrix row counts per action reflect this — absence of IST/UTC rows is deliberate, not a coverage gap.

| Action                    | TZs in matrix | Why                                                                                                            |
| ------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------- |
| WS-1 (create)             | BRT, IST, UTC | Canary — confirms API write stays TZ-independent                                                               |
| WS-2 (read + cross-layer) | BRT, IST      | IST tests FORM-BUG-7 (`#7`); UTC adds no new signal                                                            |
| WS-3 (round-trip)         | BRT           | API is passthrough; drift can't vary by server TZ                                                              |
| WS-4 (API→Forms)          | BRT, IST      | Forms pipeline is TZ-sensitive; browser-only                                                                   |
| WS-5 (format tolerance)   | BRT           | Parser is TZ-independent. DTBRT/DTIST variants test TZ offsets **inside the format**, independent of server TZ |
| WS-6 (empty/null)         | BRT           | Presence/absence; no TZ interaction                                                                            |
...
- **IST rows (2026-04-22 on):** `WS-SETUP-BASELINE IST` creates an identical all-configs record on an IST-server run. Behavior matches BRT — API write path is TZ-independent per the TZ philosophy above. FORM-BUG-7 (-1 day for Forms-UI-written records in IST) is **not** exercised by the API baseline and was never an API-layer bug; FORM-BUG-7 verification lives in WS-4 (browser-side cross-layer) and `forms-calendar/` matrix. The historical `DateTest-000084` / `#7` assertion on `ws-2-A-IST` is preserved in `analysis/ws-bug-1-cross-layer-shift.md` for context.

**Note**: Expected values below reflect the baseline-record state (all configs populated) for both BRT and IST. Pipeline writes via API; the expectations are TZ-symmetric.
...
| ws-2-A-IST |   A    | baseline (IST) | `"2026-03-15"`          | `"2026-03-15T00:00:00Z"` |  PASS  | `"2026-03-15T00:00:00Z"` |      | API write path is TZ-independent — matches BRT baseline        |
| ws-2-B-IST |   B    | baseline (IST) | `"2026-03-15"`          | `"2026-03-15T00:00:00Z"` |  PASS  | `"2026-03-15T00:00:00Z"` |      | Date-only, ignoreTimezone=true                                 |
| ws-2-C-IST |   C    | baseline (IST) | `"2026-03-15T14:30:00"` | `"2026-03-15T14:30:00Z"` |  PASS  | `"2026-03-15T14:30:00Z"` |      | DateTime — time preserved; Z appended on read                  |
| ws-2-D-IST |   D    | baseline (IST) | `"2026-03-15T14:30:00"` | `"2026-03-15T14:30:00Z"` |  PASS  | `"2026-03-15T14:30:00Z"` |      | Real Z (no fake Z — H-2 confirmed)                             |
| ws-2-E-IST |   E    | baseline (IST) | `"2026-03-15"`          | `"2026-03-15T00:00:00Z"` |  PASS  | `"2026-03-15T00:00:00Z"` |      | Legacy date-only; API returns datetime+Z                       |
| ws-2-F-IST |   F    | baseline (IST) | `"2026-03-15"`          | `"2026-03-15T00:00:00Z"` |  PASS  | `"2026-03-15T00:00:00Z"` |      | Legacy date-only + ignoreTimezone                              |
| ws-2-G-IST |   G    | baseline (IST) | `"2026-03-15T14:30:00"` | `"2026-03-15T14:30:00Z"` |  PASS  | `"2026-03-15T14:30:00Z"` |      | Legacy DateTime; time preserved                                |
| ws-2-H-IST |   H    | baseline (IST) | `"2026-03-15T14:30:00"` | `"2026-03-15T14:30:00Z"` |  PASS  | `"2026-03-15T14:30:00Z"` |      | Legacy DateTime + ignoreTimezone                               |
